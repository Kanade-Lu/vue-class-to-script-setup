...
export const traverseCode = (code) => {
  ...
  traverse(ast, {
    ...
    ObjectMethod(path) {
      if (path.node.key.name === "data") {
        try {
          ...
          body.argument.properties.forEach((item) => {
            ...
            path.parentPath.parentPath.container.splice(
              1,
              0,
              createProp(params)
            );
      ...
      if (
        ["created", "mounted", "onShow", "onLoad", "onUnload"].includes(
          path.node.key.name
        )
      ) {
        try {
          ...
          const newNode = t.arrowFunctionExpression([], body, false);
          newNode.async = node.async;
          node.async = false;
          node.params.push(newNode);
          delete node.body;
          path.parentPath.parentPath.container.push(node);
          path.node.params = [];
        } catch (error) {
          console.error(
            "['created', 'mounted', 'onShow', 'onLoad', 'onUnload'].includes(path.node.key.name)",
            error
          );
        }
      }
    ...
    ObjectProperty(path) {
      const handleActionsAndMutations = (item, type) => {
        if (
          item.argument.arguments.length === 1 &&
          t.isArrayExpression(item.argument.arguments[0])
        ) {
          const actions = item.argument.arguments[0].elements;
          actions.forEach((item) => {
            const computedNode = actionDeclaration(
              item.value,
              t.identifier("params"),
        ...
        if (
          item.argument.arguments.length === 1 &&
          t.isObjectExpression(item.argument.arguments[0])
        ) {
        ...
        if (
          ...
          t.isArrayExpression(item.argument.arguments[1])
        ) {
        ...
        if (
          item.argument.arguments.length === 2 &&
          t.isStringLiteral(item.argument.arguments[0]) &&
          t.isObjectExpression(item.argument.arguments[1])
        ) {
          const actions = item.argument.arguments[1].properties;
          const namespace = item.argument.arguments[0].value;
          actions.forEach((item) => {
            const actionsNode = actionDeclaration(
              item.value.value || item.value.name,
              t.identifier("params"),
              t.stringLiteral(
                `${namespace}/${item.value.value || item.value.name}`
              ),
              type
            );
        ...
        if (path.node.key.name === "methods") {
          path.node.value.properties.forEach((item) => {
            if (
              ...
              ["mapActions", recordNamespaceInfo.actionsName].includes(
              ...
              const newNode = t.variableDeclaration("const", [
                t.variableDeclarator(
                  t.identifier(item.key?.name || item.callee?.name || ""),

                  t.arrowFunctionExpression(
                    item.params ?? [],
                    item.body ?? t.blockStatement([]),
                    item.async
                  )
                ),
              ]);
      ...
      if (path.node.key.name === "computed") {
        path.node.value.properties?.forEach((item) => {
          ...
          if (
            t.isSpreadElement(item) &&
            t.isCallExpression(item.argument) &&
            ["mapGetters", recordNamespaceInfo.gettersName].includes(
              item.argument.callee?.name
            )
          ) {
            if (
              recordNamespaceInfo.gettersName === item.argument.callee?.name
            ) {
              ...
              getters.forEach((item) => {
                ...
                const computedNode = stateDeclaration(
                  ...
                  t.stringLiteral(
                    `${recordNamespaceInfo.namespaced}/${item.value}`
                  )
                );
      ...
      if (path.node.key.name === "props") {
        ...
        const definePropsCallExpression = t.callExpression(
  ...
  traverse(ast, {
    ...
    Program(path) {
      ...
      body.forEach((item) => {
        if (t.isImportDeclaration(item)) importList.push(item);
        ...
        if (t.isExpressionStatement(item)) {
          if (
            ...
            item.expression.callee.name === "watch"
          )
            watchList.push(item);

          if (
            t.isCallExpression(item.expression) &&
            ["onMounted", "onShow", "onLoad", "onUnload"].includes(
              item.expression.callee.name
            )
          )
          ...
          if (
            t.isCallExpression(item.expression) &&
            item.expression.callee.name === "defineProps"
          )
            definePropsList.push(item);
        }
        if (t.isCallExpression(item)) {
          if (item.callee.name === "watch") watchList.push(item);
        }
        if (t.isObjectMethod(item)) {
          if (
            ["onMounted", "onShow", "onLoad", "onUnload"].includes(
      ...
      const emitParams = t.arrayExpression(
        [...emitList].map((item) => t.stringLiteral(item))
      );
      const defineEmits = t.callExpression(t.identifier("defineEmits"), [
        emitParams,
      ]);
      const emitAst = t.variableDeclaration("const", [
        t.variableDeclarator(t.identifier("$emit"), defineEmits),
      ]);
  ...
  try {
    ...
    traverse(ast, {
      Program(path) {
        ...
        const expressionList = path.node.body.filter(
          (item) => item.type === "ExpressionStatement"
        );

        const otherBody = path.node.body.filter(
        ...
        const createReturnList = otherBody
          .filter(
            (item) =>
              ...
              t.isIdentifier(item.declarations[0].id)
          )
          .map((item) => {
            if (
              item.type === "VariableDeclaration" &&
              ...
              return t.objectProperty(
                t.identifier(item.declarations[0].id.name),
                t.identifier(item.declarations[0].id.name)
              );

            return "";
          });
        const createReturn = t.returnStatement(
          t.objectExpression(createReturnList)
        );
        path.node.body = [
          ...importList,
